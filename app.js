...
const SEED = [
  {
    ...
    services: {
      ...
      roads: {
        status: "Green",
        note: "Asphalt paving completed on Ring Road segment.",
      },
      waste: {
        status: "Green",
        note: "Collection schedule operating on time.",
      },
    },
  },
  {
    ...
    services: {
      water: {
        status: "Red",
        note: "Main trunk-line burst near Adama University — crew dispatched.",
      },
      lights: {
        status: "Green",
        note: "Grid fully operational. All zone lights active.",
      },
      roads: {
        status: "Yellow",
        note: "Pothole patching near central market crossing.",
      },
      waste: {
        status: "Green",
        note: "Routine collection completed on schedule.",
      },
    },
  },
  {
    ...
    services: {
      water: {
        status: "Green",
        note: "Pumping station running at normal capacity.",
      },
      lights: {
        status: "Red",
        note: "Transformer failure on Block 3 — maintenance crew en route.",
      },
  ...
  },
  {
    ...
    services: {
      ...
      lights: {
        status: "Green",
        note: "LED replacement program completed last week.",
      },
      roads: { status: "Green", note: "No active road reports." },
      waste: {
        status: "Yellow",
        note: "Truck breakdown — Zone B collection rescheduled for tomorrow.",
      },
    },
  },
  {
  ...
  },
  {
    ...
    services: {
      water: {
        status: "Yellow",
        note: "Lower pressure in high-altitude zones — under investigation.",
      },
      lights: { status: "Green", note: "Standard grid operational." },
      roads: {
        status: "Red",
        note: "Main river bridge structural inspection — one lane closed.",
      },
...
const State = {
  ...
  load() {
    ...
    try {
      this.cities = raw ? JSON.parse(raw) : JSON.parse(JSON.stringify(SEED));
    } catch {
      this.cities = JSON.parse(JSON.stringify(SEED));
    }
    this.theme = localStorage.getItem(THEME_KEY) || "light";
  },

  save() {
    localStorage.setItem(DB_KEY, JSON.stringify(this.cities));
  },
...
function calcHealth(services) {
  ...
  const score = keys.reduce(
    (acc, k) => acc + (vals[services[k].status] ?? 0),
    0,
  );
...
function refreshMetrics() {
  ...
  D.ringFill.style.stroke =
    pct >= 80 ? "var(--green)" : pct >= 55 ? "var(--yellow)" : "var(--red)";

  if (red === 0 && yellow === 0) {
    D.gaugeDesc.textContent =
      "All services fully operational — no outages reported.";
  } else {
...
function renderGrid() {
  ...
  const visible = State.cities.filter((c) => {
    ...
    if (filter === "Yellow" && !svcs.some((s) => s.status === "Yellow"))
      return false;
    if (filter === "Green" && !svcs.every((s) => s.status === "Green"))
      return false;
    return true;
  });
  ...
  visible.forEach((city) => {
    ...
    const cardClass = hasRed
      ? "has-outage"
      : hasYellow
        ? "has-maintenance"
        : "";

    const svcBlocks = Object.keys(svcs)
      .map((key) => {
        const svc = svcs[key];
...
function handleSvcClick(cityId, svcKey) {
  ...
  const radio = document.querySelector(
    `input[name="svc-status"][value="${svc.status}"]`,
  );
...
function bindEvents() {
  ...
  D.closePasscode.addEventListener("click", () =>
  ...
  // Close update modal
  D.closeUpdate.addEventListener("click", () =>
  ...
  D.updateForm.addEventListener("submit", (e) => {
    ...
    toast(
      ...
      status === "Red" ? "danger" : status === "Yellow" ? "warning" : "success",
    );
    refreshMetrics();
    renderGrid();
  });
